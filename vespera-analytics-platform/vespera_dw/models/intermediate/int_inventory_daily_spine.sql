...
-- x up to ~730 days each) -- normal for a daily grain fact at this
-- scale, not a bug if the row count looks large.
--
-- RESTORED -- this file's content was accidentally overwritten with
-- int_ar_invoice_daily_spine.sql's content during earlier file
-- copying. This is the original correct content, unchanged from what
-- was in the project before today's session.
